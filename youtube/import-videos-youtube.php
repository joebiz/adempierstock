...
include($root_path . "adempierstock/youtube/config_youtube.php");
...
/*
  function selectProductUrlKeyNew($productId)
  {
  //Create connection object
  global $lB, $conn,$tablePrefix;
  $productUrl = "";
  ...
  if(strpos($sqlUrlRes["request_path"], 'jewelry/') !== false)
  {
  $url = str_replace("/","-",$sqlUrlRes["request_path"]);
  $productUrlStr = $baseUrl.$url;
  $productUrl = $productUrlStr;
  }
  else
  {
                ...
                if ($fileCount <= 19) {
                    ...
                    if ($videoCode == '') {
                        ...
                        if ($data['entity_id'] != "") {
                            ...
                            if (isset($resproductvideo["value"]) && $resproductvideo["value"] != '' && $resproductvideo["value"] != 'NULL') {   //echo "case 1";die;
...
END;
                                ...
                                if (copy($root_path . "adempierstock/youtube/product_video/" . $sku . "." . $fileExtension, $root_path . "media/catalog/product/video/" . $url_key)) {
                                    ...
                                    //print_r($resproductvideo2);die;
                                    if ($resproductvideo2["value_id"] == '') {
                                        $insSQL = "INSERT INTO " . $tablePrefix . "catalog_product_entity_text(entity_type_id,store_id,attribute_id,entity_id) VALUES ('4','0','2046','" . $product_id . "')";
                                        $conn->query($insSQL);
                                        //echo "\n".$sku." Added";
                                    }
